...
(function(){
  ...
  function inferredCategories(r){
    ...
    // "Fêtes & occasions" = sweet only. Savory date-night/family dishes stay in their own categories.
    const sweetTags=(r.tags||[]).some(t=>['dessert','sweet','bakery','holiday-sweet','birthday-cake'].includes(t));
    const sweetName=/gâteau|gateau|cake|cookie|cookies|tarte|tart|brownie|muffin|cupcake|donut|beignet|macaron|biscuit|cheesecake|chocolat|chocolate|dessert|pudding|crumble|cinnamon roll|brioche sucrée|brioche sucree/.test(name);
    if((r.tags||[]).includes('occasion')&&(sweetTags||sweetName)) out.add('occasion');
    else out.delete('occasion');
