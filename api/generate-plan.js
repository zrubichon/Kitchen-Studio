...
export default async function handler(req, res) {
  ...
  const system = `You are the meal-planning engine for a premium cooking app. Return ONLY strict JSON with a key "plan" containing exactly 7 arrays, each with exactly 3 recipe IDs (breakfast,lunch,dinner). Use only these IDs: ${recipeIds.join(', ')}. Respect the supplied profile and weekly brief; prefer variety and reasonable reuse of ingredients.`;
  try {
